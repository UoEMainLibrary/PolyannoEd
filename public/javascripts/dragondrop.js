...
var isUseless = function (something) {
  if (rejectionOptions.has(something) || rejectionOptions.has(typeof something)) { return true } else { return false };
}
...
var add_dragondrop_pop = function (popupClass, contentHTML, parentID, minOption, handlebarHTML, removeCloseOption) {
  ...
  if (parentRowWidth + 4 > 12) {
    ...
    var x = document.getElementById(parentID).children[0]
    x.insertBefore(newRow, x.childNode)
  };

  var pageBody = document.getElementById(parentID).children[0].lastChild // id --> col --> rows
  pageBody.insertBefore(popupBoxDiv, pageBody.childNode)
  ...
  if (isUseless(minOption)) { $(popupIDstring).find('.dragondrop-min').detach() } else { $(popupIDstring).find('.dragondrop-min').prependTo($(popupIDstring).find('.dragondrop-handlebar')) };

  if (removeCloseOption) { $(popupIDstring).find('.dragondrop-close').detach() } else { $(popupIDstring).find('.dragondrop-close').prependTo($(popupIDstring).find('.dragondrop-handlebar')) };
...
var updateGridCols = function (newcol, popupDOM) {
  ...
  if (spaceIndex == -1) { theEndIndex = finishingIndex } else { theEndIndex = spaceIndex };
...
var formJumpArray = function (theChecks, popupDOM, nDOM, theNSiblings) {
  if (((theChecks[2] != 0) && (theChecks[3] != 0)) || ((theChecks[2] == 0) && (theChecks[3] == 0))) { /// if top and bottom are not overlapping or both are
    if (theChecks[0] != 0) { theNSiblings[0] = nDOM; return theNSiblings } else if (theChecks[1] != 0) { theNSiblings[1] = nDOM; return theNSiblings };
    ...
    if (theChecks[2] != 0) { return theNSiblings } else if (theChecks[3] != 0) { return theNSiblings };
  } else if (theChecks[0] != 0) { theNSiblings[0] = nDOM; return theNSiblings } else if (theChecks[1] != 0) { theNSiblings[1] = nDOM; return theNSiblings };
}
